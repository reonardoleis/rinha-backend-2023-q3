...
func (c *Cache) Enqueue(person []*models.Person) (bool, error) {
	var jsons []byte
	...
	err := c.client.LPush(ctx, "queue", jsons).Err()
	if err != nil {
		log.Println(err)
		return false, err
	}
...
func (c *Cache) PopFirstN(n int) ([]*models.Person, error) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*5)
	defer cancel()

	people := make([]*models.Person, 0)
	for i := 0; i < n; i++ {
		val, err := c.client.RPop(ctx, "queue").Result()
		if err != nil {
			if err == redis.Nil {
				break
			}
...
func (c *Cache) QueueSize() int {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*5)
	defer cancel()

	size, err := c.client.LLen(ctx, "queue").Result()
	if err != nil {
		log.Println(err)
		return 0
	}
